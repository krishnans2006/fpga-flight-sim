[ ECE 385 Fall 2025 ]{align="horizon"}

::: {align="horizon"}
Final Project Report
:::

[ Krishnan Shankar (ks128@illinois.edu) Aniketh Tarikonda
(aniketh8@illinois.edu) ]{align="horizon"}
...
<figure>
<p><img src="media/block_diagram.svg" style="width:100.0%" /></p>
...
<figure>
<p><img src="media/graphics_bd.png" /></p>
...
"staging" buffer. On every falling edge of

``` sv
vsync
```

, we swap the pointers to these buffers, thus allowing for consistent
...
signal refers to the **falling edge** of the actual vsync signal.
Signals
...
come from the DDR3 command FIFO, as per the light-weight controller
...
line buffer, we need 80 consecutive DRAM bursts. The

``` sv
StFlush 
```

state exists in order to avoid potential cache read/write requests from
...
<figure>
<p><img src="media/cache_fsm.png" /></p>
...
occurs at that particular line. In the event of

``` sv
StEvictWB
```

, the cache writes sequential 128-bit chunks to DDR3, of which there are
...
for brevity. In our implementation, this signal is driven by certain
...
refer to the current address tag of data in, as well as the values
stored within the writeback buffer, respectively. During

``` sv
StRead
```

, data values are being continuously read in from the rest of the
graphics pipeline. This coalesced buffer is then written to DDR3 in
...
is the default state, and none of the other states ever transition back
to it.

### zbuffer.sv

zbuffer.sv implements a Z-buffer, allowing for displaying relative depth
within our graphics pipeline. It takes in
...
values from the rasterizer, as well as the Z-values of the three
...
<figure>
<p><img src="media/rasterizer_fsm.png" /></p>
...
calculates the bounds of the smallest rectangular which fully contains
the specified triangle. These bounds are used to determine when
...
is used to wait for residual values in the address/valid pipelines to
clear, after which it transitions to
...
fractional precision required. In this excerpt,

``` sv
32'h01000000
```

is equal to decimal $1$ in Q8.24.

We determine if a point is within the triangle by checking if the sign
bit of all three areas are the same.

<figure>
<p><img src="media/barycentric_tb.png" /></p>
...
<figure>
<p><img src="media/rproj_tb.png" style="width:100.0%" /></p>
...
We can see that around $0.250\mu s$,

``` sv
proj_out_valid
```

is asserted high, indicating that the output of the projector module is
...
<figure>
<p><img src="media/example_img.png"
...
of the graphics pipeline. Internally, it instantiates two ROMs
...
, each of which are BRAM arrays holding vertex/face data in a particular
format:
...
complete interface between the FPGA and MAX3421E chip is shown in
[\[max3421e_connection\]](#max3421e_connection){.ref}. The interrupt and
...
<figure>
<p><img src="media/max3421e_connection.svg" style="width:100.0%" /></p>
...
section](https://eng.libretexts.org/Bookshelves/Aerospace_Engineering/Fundamentals_of_Aerospace_Engineering_(Arnedo)/07%3A_Mechanics_of_flight/7.01%3A_Performances/7.1.03%3A_Hypotheses)]{.underline}.
...
block design is shown in
[\[vivado_block_design\]](#vivado_block_design){.ref}.

<figure>
<p><img src="media/vivado_block_design.png" style="width:100.0%" /></p>
...
[\[block_diagram_rtl\]](#block_diagram_rtl){.ref} shows the RTL block
diagram of the FPGA implementation.

<figure>
<p><img src="media/block_rtl.png" style="height:80.0%" /></p>
...
[\[design_table\]](#design_table){.ref} shows design analysis results
for the FPGA implementation.

+-------------------------------------+-------------------------------------+
| **Utilization**                                                           |
+-------------------------------------+-------------------------------------+
| Look-Up Tables (LUTs)               | 7096 / 32600 (21.77%)               |
+-------------------------------------+-------------------------------------+
| Digital Signal Processing Units     | 71 / 120 (59.17%)                   |
| (DSPs)                              |                                     |
+-------------------------------------+-------------------------------------+
| Memory (BRAM)                       | 37.5 / 75 (50.0%)                   |
+-------------------------------------+-------------------------------------+
| Memory (LUTRAM)                     | 500 / 9600 (5.21%)                  |
+-------------------------------------+-------------------------------------+
| Latches                             | 0                                   |
+-------------------------------------+-------------------------------------+
| Flip-Flops (FFs)                    | 7493 / 65200 (11.49%)               |
+-------------------------------------+-------------------------------------+
| Input/Output (IO)                   | 88 / 210 (41.9%)                    |
+-------------------------------------+-------------------------------------+
| Mixed-Mode Clock Managers (MMCMs)   | 3 / 5 (60.0%)                       |
+-------------------------------------+-------------------------------------+
| **Timing**                                                                |
+-------------------------------------+-------------------------------------+
| Worst Negative Slack (WNS)          | -3.234 ns                           |
+-------------------------------------+-------------------------------------+
| Max Frequency                       | 75.563 MHz                          |
+-------------------------------------+-------------------------------------+
| **Power**                                                                 |
+-------------------------------------+-------------------------------------+
| Static Power                        | 7.7e-2 W                            |
+-------------------------------------+-------------------------------------+
| Dynamic Power                       | 0.953 W                             |
+-------------------------------------+-------------------------------------+
| Total Power                         | 1.03 W                              |
+-------------------------------------+-------------------------------------+
...
[<https://alchitry.com/tutorials/projects/gpu/>]{.underline}

[<https://github.com/someone755/ddr3-controller>]{.underline}

[<https://github.com/kooltzh/xilinx-coe-generator/tree/master>]{.underline}

[<https://www.cs.utexas.edu/~fussell/courses/cs384g-fall2013/lectures/lecture20-Z_buffer_pipeline.pdf>]{.underline}

[<https://github.com/sylefeb/tinygpus>]{.underline}
