...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, (u32) 0x01); // drive SS low, select SPI peripheral

	u8 write_buffer[2] = {(u8) (reg + 0x02), (u8) val};

	Status = XSpi_Transfer(&SpiInstance, write_buffer, NULL, 2); // write val

	if (Status != XST_SUCCESS) {
		xil_printf("Error -> spawned from MAXreg_wr\n");
	}

	XSpi_SetSlaveSelect(&SpiInstance, (u32) 0x00); // drive SS high, deselect SPI peripheral
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, (u32) 0x01);

	u8 write_buffer_n[nbytes + 1];
	write_buffer_n[0] = (u8) (reg + 2);

	// i think this is less overhead than memcpy/memset? idk
	for (BYTE i = 0; i < nbytes; i++) {
		write_buffer_n[i + 1] = (u8) *(data + i);
	}

	Status = XSpi_Transfer(&SpiInstance, write_buffer_n, NULL, nbytes + 1);

	if (Status != XST_SUCCESS) {
		xil_printf("Error -> Spawned from MAXbytes_wr\n");
	}

	XSpi_SetSlaveSelect(&SpiInstance, (u32) 0x00);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	u8 write_buffer = (u8) reg;
	u8 recv_buffer[2] = {0x00, 0x00};

	Status = XSpi_Transfer(&SpiInstance, &write_buffer, recv_buffer, 2);

	if (Status != XST_SUCCESS) {
		xil_printf("Error -> spawned from MAXreg_rd\n");
	}

	XSpi_SetSlaveSelect(&SpiInstance, (u32) 0x00);

	return ((BYTE) recv_buffer[1]);
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 write_buffer_n[nbytes + 1];
	u8 recv_buffer_n[nbytes + 1];

	recv_buffer_n[0] = (u8) 0x00;
	write_buffer_n[0] = (u8) reg;

	for (BYTE i = 0; i < nbytes; i++) {
		write_buffer_n[i+1] = (u8) 0x00;
		recv_buffer_n[i+1] = (u8) 0x00;
	}

	Status = XSpi_Transfer(&SpiInstance, write_buffer_n, recv_buffer_n, nbytes + 1);

	if (Status != XST_SUCCESS) {
		xil_printf("Error -> Spawned from MAXbytes_rd\n");
	}

	for (BYTE i = 0; i < nbytes; i++) {
		*(data + i) = recv_buffer_n[i+1];
	}
